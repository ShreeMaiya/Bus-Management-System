...
if (isset($_POST['sub'])) {
    ...
    $con = mysqli_connect('localhost', 'root', ' ', 'db_bus_mgm', 3308);

    // get the post records
    $pwd = $_POST['pwd'];
    $cid = $_POST['Cid'];

    // Check if the conductor username already exists
    $checkQuery = "SELECT * FROM `login` WHERE `user_name` = '$cid'";
    $checkResult = mysqli_query($con, $checkQuery);

    if (mysqli_num_rows($checkResult) > 0) {
        // Username already exists
        echo "<script>alert('Conductor Already exists!')</script>";
    } else {
        // Username does not exist, proceed with insertion
        $sql = "INSERT INTO `login`(`user_name`,`password`) VALUES('$cid', '$pwd')";
        // insert in database 

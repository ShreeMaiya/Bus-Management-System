...
<body onLoad="noBack();">
	<nav id="mainNavbar" class="navbar navbar-light navbar-expand-md py-1 px-2 fixed-top"
		...
		<div class="collapse navbar-collapse justify-content-between" id="navLinks">
			...
			<ul class="navbar-nav">
				...
				<li class="nav-item">
					<a href="../../Source/timetable.php" class="nav-link">TIME TABLE</a>
				</li>
			</ul>
			<span class="nav-item">
    ...
    if (isset($_POST['loginC'])) {
	    ...
	    $connection = mysqli_connect('localhost', 'root', ' ', 'db_bus_mgm'  );
	    if (!$connection) {
	    ...
	    if ($num == 1) {
		    ...
		    header("refresh:0, url=../../Source/conductorDashboard.php");
	    } else {
	    ...
	    if ($num == 1) {
		    ...
		    $_SESSION['status'] = "Active";
		    $_SESSION['username'] = "$username";
		    header("refresh:1, url=../../Source/Driver_DashBoard.php");
	    } else {
		    echo "<script>alert('incorrect id or password')</script>"
		    ;
		    echo "<script>location.href='logintest.php'</script>";
